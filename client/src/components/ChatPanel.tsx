...
const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  return (
    <div className="chat-container-inner h-full w-full flex flex-col glass-panel rounded-lg shadow-lg border border-muted/30 overflow-hidden">
      {/* Chat Tabs */}
      <div className="flex border-b border-muted/30 bg-muted/20 sticky top-0 z-20">
        <button 
          className={`flex-1 py-3 px-1 text-center text-xs sm:text-sm font-medium transition-all duration-200 ${
            activeTab === 'chat' 
              ? 'text-accent border-b-2 border-accent bg-muted/40' 
              : 'text-muted-foreground hover:text-foreground'
          }`}
          onClick={() => setActiveTab('chat')}
        >
          <div className="flex flex-col sm:flex-row items-center justify-center space-y-1 sm:space-y-0 sm:space-x-1.5">
            <i className="fas fa-comment-alt text-xs sm:text-sm"></i>
            <span>Chat</span>
          </div>
        </button>
        <button 
        ...
        >
          <div className="flex flex-col sm:flex-row items-center justify-center space-y-1 sm:space-y-0 sm:space-x-1.5">
            ...
            <span>Members</span>
            <span className="text-xs bg-accent/20 text-accent px-1.5 py-0.5 rounded-full">{uniqueMembers.length}</span>
          </div>
        </button>
        <button 
          className={`flex-1 py-3 px-1 text-center text-xs sm:text-sm font-medium transition-all duration-200 ${
        ...
        >
          <div className="flex flex-col sm:flex-row items-center justify-center space-y-1 sm:space-y-0 sm:space-x-1.5">
            <i className="fas fa-terminal text-xs sm:text-sm"></i>
            <span>Commands</span>
          </div>
        </button>
      </div>
      ...
      <div className="flex-grow overflow-hidden relative">
        {/* Member List Panel */}
        <div className={`absolute inset-0 h-full w-full transition-all duration-300 transform ${
          activeTab === 'members' ? 'translate-x-0 opacity-100 z-10' : 'translate-x-full opacity-0 pointer-events-none'
        }`}>
          <ScrollArea className="h-full w-full chat-scrollbar p-4">
        ...
        {/* Commands Panel */}
        <div className={`absolute inset-0 h-full w-full transition-all duration-300 transform ${
          activeTab === 'commands' ? 'translate-x-0 opacity-100 z-10' : '-translate-x-full opacity-0 pointer-events-none'
        }`}>
          <ScrollArea className="h-full w-full chat-scrollbar p-4">
              ...
              <div className="space-y-4">
                <div className="glass-panel rounded-lg p-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2 items-center">
                      ...
                      <div className="col-span-2 text-muted-foreground text-xs">Jump to timestamp</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="glass-panel rounded-lg p-3">
                  ...
                  <div className="space-y-2">
                    <div className="grid grid-cols-3 gap-2 items-center">
                      ...
                      <div className="col-span-2 text-muted-foreground text-xs">Change speed (0.25-2)</div>
                    </div>
                    <div className="grid grid-cols-3 gap-2 items-center">
                      <div className="font-mono text-foreground bg-muted/40 rounded-md px-2 py-1 text-xs">/volume 75</div>
                      <div className="col-span-2 text-muted-foreground text-xs">Set volume (0-100)</div>
                    </div>
                    <div className="grid grid-cols-3 gap-2 items-center">
                      <div className="font-mono text-foreground bg-muted/40 rounded-md px-2 py-1 text-xs">/quality hd720</div>
                      <div className="col-span-2 text-muted-foreground text-xs">Change quality</div>
                    </div>
                    <div className="grid grid-cols-3 gap-2 items-center">
                      <div className="font-mono text-foreground bg-muted/40 rounded-md px-2 py-1 text-xs">/fullscreen</div>
                      <div className="col-span-2 text-muted-foreground text-xs">Toggle fullscreen</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="text-center bg-muted/20 rounded-lg p-3">
                  <p className="text-xs text-muted-foreground">
                    Tip: You can also paste any YouTube URL directly in the chat
                  </p>
                </div>
              </div>
            </div>
        ...
        {/* Chat Messages Panel */}
        <div className={`absolute inset-0 h-full w-full transition-all duration-300 transform ${
          activeTab === 'chat' ? 'translate-x-0 opacity-100 z-10' : 'translate-x-full opacity-0 pointer-events-none'
        }`}>
          <div className="h-full flex flex-col pb-4">
            <ScrollArea className="flex-grow w-full chat-scrollbar h-[calc(100%-2rem)]">
      ...
      <div className="mt-auto py-3 px-3 border-t border-muted/30 bg-card/90 backdrop-blur-sm sticky bottom-0 left-0 right-0 z-20">
        <form onSubmit={handleSubmit} className="flex space-x-2">
          ...
          <Button type="submit" className="send-button aspect-square w-10 h-10 min-w-[2.5rem]" disabled={!message.trim()}>
